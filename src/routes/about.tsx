...
const projects = [
  {
    ...
    application:
      "Helps groups record shared expenses, understand balances, and settle debts with fewer peer-to-peer payments.",
    engineering:
      "Built with React, Node.js, Express, MongoDB/Mongoose, Passport.js, Google OAuth 2.0, JWT, and bcryptjs across independently deployed services.",
  ...
  },
  {
    ...
    application:
      "Pairs nearby travelers around shared interests and turns their preferences into a practical itinerary.",
    engineering:
      "Decoupled React SPA and Django REST API with spatial indexing, Jaccard similarity, Gemini 2.5 Flash, structured JSON, CSRF validation, and strict CORS.",
  ...
  },
  {
    ...
    application:
      "Lets candidates identify missing skills without sending resume data to an external AI provider.",
    engineering:
      "MERN architecture with local NLP, tested through Postman across the upload, analysis, and database flow.",
  ...
  },
  {
    ...
    application:
      "Converts an unstructured trip request into reusable, structured travel parameters for itinerary planning.",
    engineering:
      "Django and PostgreSQL with a custom spaCy/NLTK pipeline, Gemini integration, retry logic, and rate-limit handling.",
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-background font-body text-foreground selection:bg-lava selection:text-foreground">
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-5 py-6 sm:px-8 sm:py-9 lg:px-10">
        ...
        <section
          id="top"
          className="grid min-h-[680px] grid-cols-1 items-center gap-12 py-16 lg:grid-cols-12 lg:gap-14 lg:py-20"
        >
          <div className="lg:col-span-8">
            ...
            <p className="reveal-in reveal-delay-3 mt-6 max-w-[62ch] text-lg font-medium leading-relaxed text-muted-foreground sm:text-xl">
              I’m a final-year IT student who turns ideas into deployed products—connecting
              thoughtful interfaces, reliable APIs, practical databases, and useful AI features.
            </p>
            ...
            <div className="reveal-in reveal-delay-4 mt-8 flex flex-wrap gap-x-6 gap-y-2 text-sm font-bold text-muted-foreground">
              <span className="inline-flex items-center gap-2">
                <MapPin size={16} /> Vadodara · Open to relocation
              </span>
              <span className="inline-flex items-center gap-2">
                <Radio size={16} className="text-mint-strong" /> Available for opportunities
              </span>
            </div>
          </div>
        ...
        <RevealSection className="pt-8">
          <div className="flex items-end justify-between gap-4">
            ...
            <p className="hidden max-w-sm text-right text-sm font-semibold text-muted-foreground sm:block">
              A practical stack for designing, building, debugging, and shipping complete products.
            </p>
          </div>
            ...
            {projects.map((project, index) => (
              <article key={project.title} className="project-card group">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <span className={`rounded-full px-3 py-1 text-xs font-bold ${project.accent}`}>
                    {project.category}
                  </span>
                ...
                <div className="mt-6 flex items-start justify-between gap-4">
                  ...
                  <span className="grid size-8 shrink-0 place-items-center rounded-full bg-secondary transition-transform group-hover:-translate-y-1 group-hover:translate-x-1">
                    <ArrowUpRight size={16} />
                  </span>
                </div>
                <p className="mt-3 max-w-[55ch] text-sm font-semibold leading-relaxed text-muted-foreground">
                  {project.summary}
                </p>
                <dl className="mt-5 space-y-3 border-t border-border/70 pt-5 text-sm">
                  <div>
                    ...
                    <dd className="mt-1 font-semibold leading-relaxed text-muted-foreground">
                      {project.application}
                    </dd>
                  </div>
                  <div>
                ...
                <p className="mt-5 rounded-xl bg-secondary px-3 py-2 text-xs font-extrabold text-muted-foreground">
                  {project.proof}
                </p>
                {project.url && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="mt-4 inline-flex items-center gap-2 rounded-full bg-lava px-4 py-2 text-xs font-extrabold text-lava-foreground shadow-clay-sm transition-transform hover:-translate-y-0.5 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring"
                  >
        ...
        <RevealSection className="grid grid-cols-1 gap-4 py-12 lg:grid-cols-12">
          ...
          <article className="rounded-3xl bg-card p-7 shadow-clay lg:col-span-7 sm:p-9">
            ...
            <ul className="mt-5 space-y-3 text-sm font-semibold leading-relaxed text-muted-foreground">
              <li>
                100+ LeetCode problems solved in Java across core data structures and algorithms.
              </li>
              <li>
                62-hour full-stack development certification and NPTEL Computer Networks
                certification.
              </li>
              <li>
                Presented CashWise at Ideathon 4.0 and participated in the 2026 Environment
                Hackathon.
              </li>
            </ul>
          </article>
        ...
        <RevealSection className="my-8 flex flex-col items-start justify-between gap-6 rounded-3xl bg-primary px-7 py-9 text-primary-foreground shadow-clay sm:flex-row sm:items-center sm:px-10">
          <div>
            <p className="text-sm font-bold text-primary-foreground/60">
          ...
          <a
            href={links.email}
            className="inline-flex shrink-0 items-center gap-2 rounded-full bg-card px-5 py-3 font-display text-sm font-bold text-foreground transition-transform hover:-translate-y-1 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-card"
          >
...
function ProfileLink({
  ...
  return (
    <a
      ...
      className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-bold shadow-clay-sm transition-transform hover:-translate-y-0.5 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring ${tone}`}
    >
...
function RevealSection({
  ...
  return (
    <section id={id} className={`scroll-reveal ${className}`}>
      {children}
    </section>
  );
}
...
function ThemeToggle() {
  ...
  return (
    <button
      onClick={toggleTheme}
      className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-card text-foreground shadow-clay-sm transition-transform hover:-translate-y-0.5 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring"
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-0 overflow-hidden" aria-hidden="true">
      ...
      {[12, 27, 44, 63, 78, 91].map((left, index) => (
        <span
          key={left}
          className="signal-particle absolute size-1.5 rounded-full bg-lava/60"
          style={{
            left: `${left}%`,
            top: `${12 + index * 14}%`,
